...
def ID3(data, originaldata, features, target_attribute_name="class", parent_node_class=None):
    ...
    #OLD: If all target feature values are the same:
    #NEW: if num_instances <= min_instances:
        return r
    Else:
        pass
    If Feature_Attributes is empty:
        ...
        #OLD: Att = Attribute from Feature_Attributes with the largest information gain value
        #NEW: Att = Attribute from Feature_Attributes with the lowest weighted variance
        r = Att
        For values in Att:
            Add a new node below r where node_values = (Att == values)
            Sub_D_values = (Att == values)
            If Sub_D_values == empty:
                #OLD: Add a leaf node l where l equals the mode target value in D
                #NEW: Add a leaf node l where l equals the mean of the target values in D
            Else:
...
result = {
    'legs': {
        0: {'fins': {0.0: {'toothed': {0.0: 7.0, 1.0: 3.0}},
